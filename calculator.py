...
class Participant:
    ...
    introverted_degree: float = 0.0  # 0-1 selon Tally
...
class ParazarMatcher:
    ...
    def load_from_dataframe(self, df: pd.DataFrame) -> List[Participant]:
        ...
        try:
            ...
            participants = list(map(
                lambda row: self._create_participant_from_row(row),
                df_clean.to_dict('records')
            ))
            
            # Filtrage des participants valides (genre Tally: 'm'/'f')
            valid_participants = list(filter(
                lambda p: (
                    p.age >= 18 and p.age <= 65 and 
                    p.gender in ['M', 'F'] and
                    p.email  # Email obligatoire comme identifiant unique
                ),
                participants
            ))
    ...
    def _create_participant_from_row(self, row: Dict) -> Participant:
        ...
        try:
            # Nettoyage topics_conversations (séparés par virgules)
            topics_raw = row.get('topics_conversations', '')
            topics = list(map(str.strip, filter(None, topics_raw.split(',')))) if topics_raw else []
            
            # Nettoyage life_priorities (séparés par virgules)
            priorities_raw = row.get('life_priorities', '')
            priorities = list(map(str.strip, filter(None, priorities_raw.split(',')))) if priorities_raw else []
            
            # Normalisation du genre (Tally: 'm', 'f' -> 'M', 'F')
            ...
            return Participant(
                ...
                introverted_degree=float(row.get('introverted_degree', 0.5))  # Default 0.5 si manquant
            )
        except Exception as e:
    ...
    def _calculate_global_stats(self, groups: List[Group], unmatched: List[Participant]) -> Dict:
        ...
        return {
            ...
            "matching_rate": round(sum(len(g.participants) for g in groups) / total_participants * 100, 2) if total_participants > 0 else 0,
...
# Exemple d'utilisation avec les vrais champs Tally
if __name__ == "__main__":
    # Test avec données Tally réalistes
    test_data = pd.DataFrame([
        {
            'email': 'alice@example.com', 'first_name': 'Alice', 'age': 28, 'gender': 'f',
            'parazar_partner_id': 'PZ001', 'telephone': '0123456789', 'reservation': 'standard',
            'experience_name': 'Afterwork Parisien', 'experience_date': '2025-07-15',
            'experience_city': 'Paris', 'experience_hour': '19:00', 'job_field': 'Marketing',
            'topics_conversations': 'voyage,cuisine,art', 'relationship_status': 'célibataire',
            'life_priorities': 'carrière,famille', 'introverted_degree': 0.3
        },
        {
            'email': 'bob@example.com', 'first_name': 'Bob', 'age': 32, 'gender': 'm',
            'parazar_partner_id': 'PZ002', 'telephone': '0123456790', 'reservation': 'premium',
            'experience_name': 'Afterwork Parisien', 'experience_date': '2025-07-15',
            'experience_city': 'Paris', 'experience_hour': '19:00', 'job_field': 'Tech',
            'topics_conversations': 'tech,voyage,sport', 'relationship_status': 'célibataire',
            'life_priorities': 'équilibre,aventure', 'introverted_degree': 0.7
        },
        {
            'email': 'clara@example.com', 'first_name': 'Clara', 'age': 26, 'gender': 'f',
            'parazar_partner_id': 'PZ003', 'telephone': '0123456791', 'reservation': 'standard',
            'experience_name': 'Afterwork Parisien', 'experience_date': '2025-07-15',
            'experience_city': 'Paris', 'experience_hour': '19:00', 'job_field': 'Design',
            'topics_conversations': 'art,musique,nature', 'relationship_status': 'en couple',
            'life_priorities': 'créativité,amitié', 'introverted_degree': 0.4
        },
        {
            'email': 'david@example.com', 'first_name': 'David', 'age': 35, 'gender': 'm',
            'parazar_partner_id': 'PZ004', 'telephone': '0123456792', 'reservation': 'premium',
            'experience_name': 'Afterwork Parisien', 'experience_date': '2025-07-15',
            'experience_city': 'Paris', 'experience_hour': '19:00', 'job_field': 'Finance',
            'topics_conversations': 'finance,sport,voyage', 'relationship_status': 'célibataire',
            'life_priorities': 'succès,famille', 'introverted_degree': 0.2
        }
    ])
    
    matcher = ParazarMatcher()
    participants = matcher.load_from_dataframe(test_data)
    status, results = matcher.create_optimal_groups(participants)
    
    print(f"Status: {status}")
    print(f"Participants chargés: {len(participants)}")
    print(f"Statistiques: {results.get('stats', {})}")
    
    # Affichage des groupes créés
    if results.get('groups'):
        for i, group in enumerate(results['groups']):
            print(f"\nGroupe {i+1} ({group.id}):")
            print(f"  Expérience: {group.experience_name}")
            print(f"  Participants: {len(group.participants)}")
            for p in group.participants:
                print(f"    - {p.first_name} ({p.gender}, {p.age}ans) - {p.job_field}")
                print(f"      Topics: {', '.join(p.topics_conversations)}")
                print(f"      Introversion: {p.introverted_degree}")